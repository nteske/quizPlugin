...
buildfire.components.SortableList = class SortableList {
    ...
    constructor(element, items = [], useFor) {
    ...
    injectItemElements(item, index, divRow) {
        ...
        edit.className = "btn btn--icon icon icon-pencil";
        ...
        if (this.useFor) divRow.appendChild(edit);
        ...
        deleteButton.onclick = () => {
            ...
            this.onDeleteItem(item, index, confirmed => {
                buildfire.messaging.sendMessageToWidget({ deletedItem: item});
                if (confirmed) {
    ...
    onEditItem(item, index) {
            //console.error("please handle onEditItem", item);
            let a = document.createElement('a');
            a.href = './questionForm.html?question=' + JSON.stringify(item);
            a.click();
        }
        // This will be triggered when you delete an item
